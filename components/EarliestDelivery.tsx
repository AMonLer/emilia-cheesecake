...
export default function EarliestDelivery({ className = "", tone = "dark" }: { className?: string; tone?: "dark" | "light" }) {
  ...
  return (
    <p className={`flex items-center gap-2 ${className}`}>
      <Truck className={`h-4 w-4 shrink-0 ${tone === "light" ? "text-white" : "text-[#651A1A]"}`} strokeWidth={1.75} aria-hidden="true" />
      <span>
        {t.delivery.earliest}:{" "}
        <strong className={`font-bold ${tone === "light" ? "text-white" : "text-[#651A1A]"}`}>
